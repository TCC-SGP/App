...
import {
    ...
    SafeAreaView, Keyboard, FlatList, ScrollView, Image, Modal, Pressable
} from "react-native";
...
export default function Listar_pets({ navigation }) {
    ...
    async function deletar(id_) {
        try {
            const res = await axios.get(
                "http://192.168.183.61/api_sgpApp/deletar_pet.php?id=" + id_);
    ...
    async function listarDados() {
        try {
            ...
            const res = await axios.get("http://192.168.183.61/api_sgpApp/listar_pet.php");
            setLista(res.data.result);
            Keyboard.dismiss();
        } catch (error) {
            console.log('Erro: ', error);
            ...
            <ScrollView>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => <Listagem data={item}
                        deleteItem={deletar}
                        editItem={(data) => navigation.navigate("Editar Pets", {data})} />}
                />
